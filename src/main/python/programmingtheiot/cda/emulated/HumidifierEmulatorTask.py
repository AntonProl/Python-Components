...
class HumidifierEmulatorTask(BaseActuatorSimTask):
	...
	def _activateActuator(self, val: float = ConfigConst.DEFAULT_VAL, stateData: str = None) -> int:
		if self.sh.screen:
			msg = (self.getSimpleName() or "Unknown") + ' ON: ' + str(val) + 'C'
	...
	def _deactivateActuator(self, val: float = ConfigConst.DEFAULT_VAL, stateData: str = None) -> int:
		if self.sh.screen:
			msg = (self.getSimpleName() or "UnknownActuator") + ' OFF'

			self.sh.screen.scroll_text(msg)
	
			# opción de dormir (5 segundos) para que el mensaje se desplace antes de limpiar la pantalla
			sleep(5)
